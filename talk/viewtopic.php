...
$query = array(
	'SELECT'	=> 'u.email, u.title, u.url, u.location, u.signature, u.email_setting, u.num_tips, u.num_posts, u.registered, u.admin_note, p.id, p.poster AS username, p.poster_id, p.poster_ip, p.poster_email, p.message, p.hide_smilies, p.posted, p.edited, p.edited_by, g.g_id, g.g_user_title, o.user_id AS is_online',
	'FROM'		=> 'posts AS p',
	'JOINS'		=> array(
		array(
...
while ($cur_post = $forum_db->fetch_assoc($result))
{
	...
	else
	{
		...
		if ($cur_post['poster_id'] > 1)
		{
			...
			{
				$forum_page['avatar_markup'] = "<img src=\"http://www.gravatar.com/avatar/".md5(strtolower(trim($cur_post['email'])))."?d=".urlencode("http://loungetips.com/images/defaultavatar.jpg")."&s=100\" alt=\"\" />";
...
*/
				if ($forum_config['o_show_post_count'] == '1' || $forum_user['is_admmod'])
					$forum_page['author_info']['tips'] = '<li><span>'.$lang_topic['Tips info'].' <strong>'.forum_number_format($cur_post['num_tips']).'</strong></span></li>';
