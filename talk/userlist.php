...
// Grab the users
$query = array(
	'SELECT'	=> 'u.id, u.username, u.title, u.num_tips, u.num_posts, u.registered, g.g_id, g.g_user_title',
	'FROM'		=> 'users AS u',
	'JOINS'		=> array(
		array(
...
<?php
	...
	while ($user_data = $forum_db->fetch_assoc($result))
	{
		...
		if ($forum_page['show_post_count'])
			$forum_page['table_row']['posts'] = '<td class="tc'.count($forum_page['table_row']).'">'.forum_number_format($user_data['num_tips']).'</td>';
